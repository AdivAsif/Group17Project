...
public class ExceptionMiddleware
{
    ...
    private static Task HandleExceptionAsync(HttpContext context, ILogger logger, Exception ex)
    {
        ...
        switch (ex)
        {
            case ProfileException profileEx:
                var authError = new ErrorDetails
                {
                    Message = profileEx.Message,
                    UserMessage = profileEx.UserMessage,
                    InnerMessage = profileEx.InnerException?.Message
                };

                envelope = new ResponseEnvelope<object>(authError, (int) profileEx.HttpStatusCode);
                context.Response.StatusCode = (int) profileEx.HttpStatusCode;
